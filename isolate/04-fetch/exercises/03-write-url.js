...
const testWormsTypes = (wormsTypes) => {
	...
	it('these worms can be found in Siberia', () => {
		expect(wormsTypes).to.deep.equal({
			striped: ['siberia', 'europe', 'antarctica', 'pacific islands', 'indian subcontinent'],
...
fetch(requestURL).then(parseResponse).then(separateAustralianTypes).then(testWormsTypes).catch(handleRejection);
